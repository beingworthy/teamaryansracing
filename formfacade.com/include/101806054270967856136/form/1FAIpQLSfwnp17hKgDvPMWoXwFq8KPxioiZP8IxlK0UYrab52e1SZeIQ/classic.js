(function(f) {
    if (typeof exports === "object" && typeof module !== "undefined" && false) {
        module.exports = f()
...
function FormFacade(data) {
    ...
    this.computeField = function(tmpl = '', citm) {
        ...
        return this.calculateEngine(tmpl, {
            calcfield: citm
        });
    }
    ...
    this.init = function(savedId) {
        ...
        if (prefillId) {
            var prefillprm = this.fetchPrefill(publishId, prefillId).then(jso => jso.entry ? {
                entry: jso.entry
            } : {});
            prms.push(prefillprm);
        } else if (copyId) {
            var copyprm = this.fetchResponse(publishId, copyId).then(jso => jso.entry ? {
                entry: jso.entry
            } : {});
            prms.push(copyprm);
        } else if (contactId) {
            var contactprm = this.fetchContact(userId, publishId, contactId).then(jso => jso.entry ? {
                entry: jso.entry
            } : {});
        ...
        if (window.posMode && curr.data.request.params.userId) {
            try {
                curr.posMode = new POS(curr.data.request.params.userId, {
                    authentication: {
                        userId: curr.data.request.params.userId
                    }
                });
            } catch (error) {
                console.error(error);
        ...
        return Promise.all(prms).then(function(rs) {
            ...
            curr.data.scraped = curr.data.ban ? {
                error: 'Not public',
                errorMessage: curr.data.ban
            } : scraped;
            curr.data.form = form;
    ...
    this.renderSignature = function() {
        ...
        signatures.forEach(function(signature) {
            ...
            if (!signature.dataset.esignAdded && canvas) {
                ...
                curr.signaturePad[itemId] = new SignaturePad(canvas, {
                    itemId: itemId
                });
                ...
                signaturePad.addEventListener("endStroke", (event) => {
                    ...
                    if (!params.publishId) {
                        var data = {
                            publishId: publishId,
                            currParams: curr.data.request.params,
                            params: params,
                            userAgent: navigator.userAgent
                        }
                        curr.showWarning('Signature undefined issue', data, null, {
                            ignorePopup: true
                        });
                    }
                });
                ...
                if (signaturePad && signaturePad.isEmpty() && curr.signatures.signs[entryId]) {
                    signaturePad.fromDataURL(curr.signatures.signs[entryId], {
    ...
    this.saveSignImage = function(itemId, entryId) {
        ...
        curr.saving = prm.then(function(svid) {
            ...
            var data = {
                image: curr.signaturePad[itemId].toDataURL()
            };
    ...
    this.renderUploadField = function(id, entry, files) {
        ...
        var uppyopts = {
            ...
            autoProceed: true,
            restrictions: {
                maxFileSize: filemb * 1024 * 1024,
                maxNumberOfFiles: maxnum,
                minNumberOfFiles: minnum
            },
        ...
        if (exts != 'all')
            uppyopts.restrictions.allowedFileTypes = exts.split(',').map(function(ext) {
                return '.' + ext.trim();
            });
            ...
            window.loadingScripts.then(_ => {
                pypanes.forEach(pyp => {
                    ...
                    var opts = {
                        ...
                        userId: curr.data.request.params.userId,
                        wallet: wallet,
                        orderId: curr.draft.savedId,
                        amount: amount,
                        currency: stg.currency,
                        currencyCode: stg.currencyCode,
                        to: to,
                        toname: curr.config.title || curr.data.scraped.title,
                        note: note
                    };
    ...
    this.showAll = function() {
        var doc = this.getDocument();
        doc.querySelectorAll('.ff-section').forEach(function(sec) {
    ...
    this.getCartItems = function() {
        ...
        var lines = curr.calculateEngine(billfn, {
            returntype: true
        });
...
<div class="ff-cta-center" id="ff-download-mobile-app" style="margin-top:40px;">
    ...
    this.submitData = function(nmval) {
        var pairs = {
            id: this.data.request.params.publishId
        };
        ...
        if (itmlen > 0) {
            ...
            this.__sections.forEach((sec, s) => {
                sec.items.filter(itm => {
                    ...
                    item.product = (item.price.min > 0 || item.price.max > 0) && item.titleImage ? {
                        i: itmi
                    } : null;
                    if (oitem.widget == 'product' || oitem.prdimage) item.product = {
                        i: itmi,
                        noun: fac.setting.noun
                    };
                    if (item.product) {
                            ...
                            if (curr.data.fulledit) {
                                ...
                                if (curr.data.officeuseSectionId == headerId) // current officeuse section
                                    ...
                                    if (curr.data.facade.titles && curr.data.facade.titles[headerId]) {
                                        ...
                                        curr.data.facade.titles[headerId] = {
                                            messageMark: '-- Not yet completed --'
                                        }
                                    }
                                }
    ...
    this.splitHeaders = function(headers) {
        ...
        headers.forEach(header => {
            ...
            var inHeader = Object.assign({}, header, {
                items: inItems
            });
            ...
            if (outItems.length > 0) {
                ...
                var outHead = {
                    outstock: 'out-' + outItem.id,
                    title: curr.lang('Out of stock'),
    ...
    this.scrollAdditionalImage = (type) => {
        if (type === 'down') {
            ...
            document.querySelector('.ff-prdimglast').scrollIntoView({
                behavior: "smooth",
                block: "end",
                inline: "nearest"
            });
            ...
            // scroll to top
            document.querySelector('.ff-prdimgfirst').scrollIntoView({
                behavior: "smooth",
                block: "start",
                inline: "nearest"
            });
    ...
    this.uploadFile = function(fld, entry, widg) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            if (hdn) {
                hdn.value = jso.file;
                hdn.dispatchEvent(new Event('change', {
                    bubbles: true
                }));
            }
    ...
    this.getPaymentButtons = function() {
        ...
        for (var secid in sbmt) {
            ...
            if (secbtn.payConfig == 'peergateway') {
                secbtn.id = secid;
                paybtns.push(secbtn);
            } else if (secbtn.payConfig == 'disabled') {} else if (secbtn.amountFrom) {
                secbtn.id = secid;
                paybtns.push(secbtn);
            }
        }
    ...
    this.showPayment = function(frm, secid) {
        ...
        if (itm && itm.entry) {
            ...
            if (isNaN(amt)) {
                ...
                this.showWarning('Invalid amount configuration', 'You have mapped a text field for Amount. Please select the correct field for the Amount option.', null, {
                    ignorePopup: true
                });
            ...
            this.showWarning('Invalid amount configuration', `You have not mapped the amount field required for payment. Please select the correct field for the ${errText} option`, null, {
                ignorePopup: true
            });
            elms.forEach(elm => elm.src = 'https://formfacade.com/img/send.svg');
            return false;
    ...
    this.showPeerPayment = function(secid, amt) {
        ...
        Promise.resolve(window.PaymentForm || this.loadScripts(scripts)).then(_ => {
            ...
            prm.then(function(svid) {
                ...
                var lineItems = (curr.calculateEngine("${getBill()}", {
                    returntype: true
                }) || [])[0]
                lineItems = curr.calculateEngine("${getBill()}", {
                    returntype: true
                }).map(function(item) {
                    return {
                        name: item[0],
                ...
                var params = {
                    app: 'neartail',
                    userId: userId,
                    configId: sbmt.configId,
                    currency: currency,
                    currencyCode: currencyCode,
                    publishId: curr.data.request.params.publishId,
                    lineItems: lineItems
                };
                window.paymentForm = new PaymentForm(params, {
                    ...
                    walletSelectCallback: function(rs) {
                        ...
                        var callback = function(rs) {
                            ...
                            window.paymentForm.loadPeerGatewayWallets({
                                note: note,
                                orderId: curr.draft.savedId
                            });
                            return new Promise(function(resolve, reject) {
    ...
    this.showPaymentMethod = function(idx, secid) {
        ...
        if (curr.data.facade.mapping) {
            ...
            methopts.payment_method.billing_details = {
                name,
                email,
                phone
            };
        ...
        if (meth.mount) {
            ...
            card = elements.create(meth.mount, {
                style: style
            });
    ...
    this.consentDialog = function(submitSec) {
        ...
        if (isDefaultConsent) {
            ...
            prm.then(function(svid) {
                if (!svid) throw Error('Save failed! Try again.');
                fetch(`${baseurl}/draft/${publishId}/summary/${savedId}`, {
                    method: "GET"
                }).then(function(result) {
                    return result.text();
                }).then(function(data) {
            ...
            fetch(`${baseurl}/consent/docs/${submitSec.template}`, {
                    method: "GET"
                })
                ...
                .then(function(jso) {
                    ...
                    curr.showConsentDialog(html);
                    if (jso.margin)
                        $(".ff-consent-start").css({
                            padding: `${jso.margin.top} ${jso.margin.right} ${jso.margin.bottom} ${jso.margin.left}`
                        });
                });
        }
    }
    ...
    this.cancelComboProductSelection = function(id, val, index) {
        ...
        var lines = this.calculateEngine("${getBill()}", {
            returntype: true
        });
        let _ = {};
...
FormFacade.prototype.asSections = function(frm) {
    ...
    var section = {
        ...
        titleMark: frm.titleMark,
        helpMark: frm.helpMark
    };
    ...
    sitems.forEach(function(sitem) {
        if (sitem.type == 'PAGE_BREAK') {
            ...
            section = {
                ...
                headers: [],
                items: [],
                titleMark: sitem.titleMark,
                helpMark: sitem.helpMark
            };
...
FormFacade.prototype.calculateEngine = function(tmpl, opts = {}) {
    ...
    var toDate = function(dt) {
        ...
        if (citm) dt.getMetadata = function() {
            return citm;
        };
        dt.valueOf = function() {
            return this.getTime();
    ...
    params.pretty = params.PRETTY = function() {
        var args = Array.prototype.slice.call(arguments);
        var fargs = args.filter(function(ar) {
            return ar;
        });
    ...
    params.quantity = params.QUANTITY = function(currency, ctg) {
        ...
        lines.forEach(line => {
            ...
            var fcitm = fcitms[id] || {};
            if (fcitm.configurable == 'Modifier') {} else if (ctg) {
                if (ctg == section)
    ...
    params.textsummary = params.TEXTSUMMARY = function(pattern, all = true, wa = '') {
        ...
        itms.forEach(function(item, itmi) {
            ...
            if (citm && citm.id == item.id) {} else if (item.type == 'GRID' && val) {
                var valids = val ? val.filter(function(vl) {
            ...
            var tfoot = args.map(function(foot, l) {
                ...
                if (foot.getMetadata && foot.getMetadata()) {
                    ...
                    if (meta.mapped == 'amount' && estamt > foot) {
                        ...
                        var langed = curr.lang('You saved $amount', {
                            amount: saved
                        });
...
window.formFacade = new FormFacade({
    "request": {
        "params": {
            "publishId": "1FAIpQLSfwnp17hKgDvPMWoXwFq8KPxioiZP8IxlK0UYrab52e1SZeIQ",
            "target": "classic",
            "userId": "101806054270967856136"
        },
        "query": {
            "div": "ff-compose"
        }
...
formFacade.template = {
    ...
    "preview": "\n<style>\n\t:root{\n\t\t--ff-bgcolor: #fff;\n\t\t--ff-font-color: #202124;\n\t    --ff-font-size: 14px;\n\t    --ff-head-size: 31.5px;\n\t    --ff-font-small: 12px;\n\t    --ff-field-bgcolor: #fff;\n\t\t--ff-gray-900: #202124;\n\t}\n    .ff-preview .ff-blur{ filter:blur(3px); -webkit-filter:blur(3px); }\n</style>\n\n<form id=\"Publish_preview\" class=\"ff-form ff-layout-default ff-public-mode ff-preview\">\n\n    <div class=\"ff-section\" id=\"ff-sec-root\">\n\n\t\t\n\t\t\t<img src=\"https://lh5.googleusercontent.com/X0uGfNo7NIsE_xlPVR7ecnHAMtAx17c5vWY_mESudhAzB2dc2yp8B4eAF5GMradPlcfLxoc8YxoSLP8wTeQm1oJStLqJU1EWuz-58md0QrvKcPxCGWxRoFm3lr3EFEmIMw=w1408\" class=\"ff-banner-image ff-image\">\n\t\t\n\n\t\t<h3 class=\"h3 ff-title \" id=\"ff-title-root\">\n\t\t\tVJTI Racing Crowdfunding Campaign 2.0\n\t\t</h3>\n      \n        <div class=\"ff-description \" id=\"ff-desc-root\">\n        \tA one of a kind opportunity to support your beloved automobile club, and help us build the best vehicle we can!\n\nThank you for being part of the VJTI Racing's crowdfunding campaign!!✨✨\nYour support keeps our teams fueled up as we get ready to showcase our excellence on the national stage 🏎🔥\n\nFor more information on our team and our history, please visit https://vjtiracing.in/\n        </div>\n      \n    \t<div class=\"ff-secfields\">\n    \t\t<img src=\"https://formfacade.com/banner/loadingform.gif\" loading=\"lazy\" class=\"ff-title-image ff-image\">\n        </div>\n\n    </div>\n\n</form>\n"
}
...
formFacade.config = {
    ...
    "trial": 302,
    "setting": {},
    "installAt": 1698760146590,
    "themecolor": "colorful-5d33fb",
    "themecss": "font=%22Roboto%22%2C%20sans-serif&heading=%22Poppins%22%2C%20sans-serif&primary=%235d33fb&primaryActive=%23492bbb&secondary=%23b161fc",
    "shades": {
    ...
    "originTime": 1724864857140,
    "originId": "f3817b41fa0e1e22518312349752d23e05b76874aee30d5128539fd188545e44"
}
